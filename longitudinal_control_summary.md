...
3.  **Planner Update (`LongitudinalPlanner.update`)**:
    ...
    *   **Jerk Cost & Follow Time Calculation (`frogpilot_following.py`)**: Before being used by the planner, the jerk costs and follow time (`tFollow`) are calculated by the `FrogPilotFollowing` class based on the current state and FrogPilot toggles. These values are presumably published within the `frogpilotPlan` message which the planner reads.
        *   **Base Values**:
            *   If `frogpilotCarState.trafficModeActive` is `True`, jerk costs (`base_acceleration_jerk`, `base_speed_jerk`, `base_danger_jerk`) and `t_follow` are interpolated based on `v_ego` using specific `traffic_mode_...` toggle values. Separate acceleration/deceleration jerk values are used based on `aEgo`.
            *   If `trafficModeActive` is `False`, base jerk costs and `t_follow` are determined using the standard `get_jerk_factor` and `get_T_FOLLOW` functions based on the selected `controlsState.personality` (Aggressive, Standard, Relaxed) and the corresponding toggle values (e.g., `aggressive_jerk_acceleration`, `standard_follow`). Separate acceleration/deceleration jerk values are used based on `aEgo`.
        *   **Dynamic Adjustments (Lead Following)**: If a lead is being tracked (`self.frogpilot_planner.tracking_lead` is True):
            *   **Human Following (Faster Lead)**: If `frogpilot_toggles.human_following` is enabled and `v_lead > v_ego`, the `acceleration_jerk`, `speed_jerk`, and `t_follow` values are dynamically *reduced* (divided by `acceleration_offset`). This offset increases as the speed difference (`v_lead - v_ego`) grows and decreases as the distance gap increases, aiming for a more natural approach to a faster lead.
            *   **Human Following / Conditional Slower Lead (Slower Lead)**: If (`frogpilot_toggles.conditional_slower_lead` or `frogpilot_toggles.human_following`) is enabled and `v_lead < v_ego`:
                *   A `braking_offset` is calculated, increasing as the speed difference (`v_ego - v_lead`) grows and decreasing as the distance gap increases.
                *   If `frogpilot_toggles.human_following` is enabled, `t_follow` is dynamically *reduced* (divided by `braking_offset`).
                *   A boolean flag `slower_lead` is set based on this offset, potentially influencing other logic (though not directly used in the planner's MPC cost calculations shown here).
        *   **Final Values**: The potentially adjusted `acceleration_jerk`, `danger_jerk`, `speed_jerk`, and `t_follow` are stored within the `FrogPilotFollowing` instance and presumably used to populate the `frogpilotPlan` message fields read by the `LongitudinalPlanner`.
