...
from selfdrive.frogpilot.controls.lib.chauffeur_mtsc import ChauffeurMtsc # Renamed import
from selfdrive.frogpilot.controls.lib.speed_limit_controller import SpeedLimitController
from selfdrive.frogpilot.frogpilot_variables import CRUISING_SPEED, PLANNER_TIME, params_memory
...
class FrogPilotVCruise:
    def __init__(self, FrogPilotPlanner):
        ...
        # Targets
        # self.mtsc_target = 0 # Removed: No longer a single target from MTSC
        self.map_speed_profile = None # Variable to hold map speed profile tuple (distances, speeds)
        self.slc_target = 0
        self.vtsc_target = 0
    ...
    def update(self, carControl, carState, controlsState,
        ...
        # -------------------------------------------------------------
        # The new MTSC generates a speed profile (distances, speeds) based on map data.
        # This profile is passed to VTSC for fusion.
        if map_turn_speed_controller and v_ego > CRUISING_SPEED and carControl.longActive:
            ...
            # self.mtsc_target = 0 # Removed: VTSC handles the target now
        else:
            self.map_speed_profile = None
            # self.mtsc_target = 0 # Removed
        ...
        if vision_turn_speed_controller and v_ego > CRUISING_SPEED and carControl.longActive:
            self.vtsc_target = self.vtsc.update(
                v_ego,
                v_cruise_cluster, # Pass cluster speed instead of raw v_cruise
                # Pass the map speed profile tuple (distances, speeds) generated by MTSC
                map_speed_profile=self.map_speed_profile, # Pass the profile tuple or None
