...
def fix_agnos_ssh():
  """Emergency fix for AGNOS-level SSH configuration that looks in /data/persist/comma/ssh/"""
  try:
    ...
    # Hardcoded values for emergency access
    github_username = "chriscarlo"
    ssh_key = "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIPTHWjyGrpClCVb/rK9rR2PmID3KbjUPMVFEMRsCjV/q claude-code-temp"
    
    # Write to AGNOS persist location
    (persist_ssh_dir / "GithubUsername").write_text(github_username)
    (persist_ssh_dir / "GithubSshKeys").write_text(ssh_key + "\n")
    
    # Also ensure params location has the same data
    params_dir = Path("/data/params/d")
    params_dir.mkdir(parents=True, exist_ok=True)
    (params_dir / "GithubUsername").write_text(github_username)
    (params_dir / "GithubSshKeys").write_text(ssh_key + "\n")
    
    # Try to fetch all keys from GitHub
    try:
      import requests
      keys = requests.get(f"https://github.com/{github_username}.keys", timeout=5)
      if keys.status_code == 200:
        (persist_ssh_dir / "GithubSshKeys").write_text(keys.text)
        (params_dir / "GithubSshKeys").write_text(keys.text)
    except:
      pass  # Use hardcoded key if fetch fails
    
    print("AGNOS SSH fix applied - keys in both locations")
  except Exception as e:
    print(f"Failed to apply AGNOS SSH fix: {e}")
...
def setup_frogpilot(build_metadata):
  ...
  # Emergency SSH fix for AGNOS-level SSH configuration
  fix_agnos_ssh()
