...
class MapReader:
    def __init__(self, cache_size: int = DEFAULT_CACHE_SIZE, worker_cpu: int | None = None):
        log_event_reader("READER", "INFO", "INIT_START", cache_size=cache_size, worker_cpu=worker_cpu if worker_cpu is not None else "None")
        ...
        if worker_cpu is not None:
            try:
                ...
                if native_id is not None:
                    ...
                    log_event_reader("READER", "INFO", "WORKER_CPU_AFFINITY_SUCCESS", worker_cpu=worker_cpu, native_id=native_id)
                ...
                log_event_reader("READER", "WARN", "WORKER_CPU_AFFINITY_NOT_SUPPORTED")
                ...
                log_event_reader("READER", "WARN", "WORKER_CPU_AFFINITY_PERMISSION_ERROR", error=str(e))
            except Exception as e:
                # print(f"MapReader: Unexpected error while setting affinity: {e}")
                log_event_reader("READER", "ERROR", "WORKER_CPU_AFFINITY_EXCEPTION", error=str(e))

        self._determine_initial_region()
        # print(f"MapReader Initialized (Cache Size: {self.cache_size}). Ready to load tiles on demand.")
        log_event_reader("READER", "INFO", "INIT_COMPLETE", cache_size=self.cache_size, current_region=self.current_region if self.current_region else "None")

    def _determine_initial_region(self):
        # print("MapReader: Determining initial region...")
        log_event_reader("READER", "DEBUG", "DETERMINE_INITIAL_REGION_START")
        params_memory = None
        last_gps = None
        try:
            ...
            if gps_json:
                ...
                if 'latitude' in last_gps and 'longitude' in last_gps:
                    ...
                    log_event_reader("READER", "INFO", "INITIAL_REGION_GPS_DATA", latitude=lat, longitude=lon)
                    for region, bounds in REGION_BOUNDS.items():
                        ...
                        if min_lon_b <= lon <= max_lon_b and min_lat_b <= lat <= max_lat_b:
                            ...
                            log_event_reader("READER", "INFO", "INITIAL_REGION_SET", region=self.current_region)
                            break
                    if self.current_region is None:
                        # print("MapReader: GPS location not in known regions.")
                        log_event_reader("READER", "WARN", "INITIAL_REGION_GPS_OUTSIDE_BOUNDS")
                else:
                    # print("MapReader: GPS data missing lat/lon.")
                    log_event_reader("READER", "WARN", "INITIAL_REGION_GPS_DATA_INVALID")
            else:
                # print("MapReader: No initial GPS data found in params.")
                log_event_reader("READER", "INFO", "INITIAL_REGION_NO_GPS_PARAMS")
        except Exception as e:
            # print(f"MapReader: Error reading LastGPSPosition for initial region: {e}")
            log_event_reader("READER", "ERROR", "INITIAL_REGION_PARAM_READ_EXCEPTION", error=str(e))

        if self.current_region is None:
            # print("MapReader: Could not determine initial region.")
            log_event_reader("READER", "WARN", "INITIAL_REGION_NOT_DETERMINED")
        log_event_reader("READER", "DEBUG", "DETERMINE_INITIAL_REGION_END", region_determined=self.current_region if self.current_region else "None")

    def _get_tile_path_for_worker(self, tile_id):
        """Gets tile path. Attempts to infer region from tile ID if needed."""
        log_event_reader("READER_WORKER", "DEBUG", "GET_TILE_PATH_START", tile_id=tile_id)
        try:
            ...
            if not determined_region:
                 log_event_reader("READER_WORKER", "DEBUG", "TILE_PATH_REGION_INFERENCE_START", tile_lat=lat_val, tile_lon=lon_val)
                 for region, bounds in REGION_BOUNDS.items():
                      min_lon_b, min_lat_b, max_lon_b, max_lat_b = bounds
                      if min_lon_b <= lon_val <= max_lon_b and min_lat_b <= lat_val <= max_lat_b:
                          determined_region = region
                          log_event_reader("READER_WORKER", "INFO", "TILE_PATH_REGION_INFERRED", tile_id=tile_id, region=determined_region)
                          break
                 if not determined_region:
                     log_event_reader("READER_WORKER", "WARN", "TILE_PATH_REGION_INFERENCE_FAIL", tile_id=tile_id, current_map_region=self.current_region if self.current_region else "None")

            if not determined_region:
                # print(f"Warning: Cannot get tile path for {tile_id}, region not determined.")
                log_event_reader("READER_WORKER", "WARN", "GET_TILE_PATH_FAIL_NO_REGION", tile_id=tile_id)
                return None
            ...
            log_event_reader("READER_WORKER", "DEBUG", "GET_TILE_PATH_SUCCESS", tile_id=tile_id, path=tile_file_path, region=determined_region)
            return tile_file_path

        except Exception as e:
            # print(f"Error parsing tile_id '{tile_id}' for path: {e}")
            log_event_reader("READER_WORKER", "ERROR", "GET_TILE_PATH_EXCEPTION", tile_id=tile_id, error=str(e))
            return None
    ...
    def _tile_loader_worker(self):
        ...
        log_event_reader("READER_WORKER", "INFO", "WORKER_THREAD_STARTED")
        while True:
            tile_id = self.load_queue.get()
            log_event_reader("READER_WORKER", "INFO", "TILE_LOAD_START", tile_id=tile_id, queue_size=self.load_queue.qsize())
            ...
            if not tile_path_proto:
                ...
                log_event_reader("READER_WORKER", "ERROR", "TILE_LOAD_FAIL_NO_PATH", tile_id=tile_id)
                with self.loading_lock:
                    if tile_id in self.queued_or_loading:
                        self.queued_or_loading.remove(tile_id)
                        log_event_reader("READER_WORKER", "DEBUG", "TILE_REMOVED_FROM_QUEUED_OR_LOADING", tile_id=tile_id, reason="no_path")
            ...
            try:
                ...
                log_event_reader("READER_WORKER", "WARN", "TILE_LOAD_FILE_NOT_FOUND", tile_id=tile_id, proto_path=tile_path_proto, idx_path=tile_path_idx)
                tile_processed_successfully = False
            except Exception as e:
                # print(f"Worker: Error processing tile {tile_id}: {e}")
                log_event_reader("READER_WORKER", "ERROR", "TILE_LOAD_PROCESSING_EXCEPTION", tile_id=tile_id, error=str(e))
                tile_processed_successfully = False
            ...
            if segments_to_cache_local:
                with self.loading_lock:
                    log_event_reader("READER_WORKER", "DEBUG", "CACHE_UPDATE_START", tile_id=tile_id, num_segments_to_add=len(segments_to_cache_local), current_cache_size=len(self.segments_data))
                    ...
                    if evicted_ids_log:
                        log_event_reader("READER_WORKER", "INFO", "CACHE_EVICTION", tile_id=tile_id, num_evicted=len(evicted_ids_log), evicted_ids=evicted_ids_log, new_cache_size=len(self.segments_data))
                    log_event_reader("READER_WORKER", "DEBUG", "CACHE_UPDATE_END", tile_id=tile_id, loaded_for_this_tile=loaded_count_for_tile, new_cache_size=len(self.segments_data))

                if loaded_count_for_tile > 0:
                    log_event_reader("READER_WORKER", "INFO", "TILE_LOAD_SUCCESS_SEGMENTS_CACHED", tile_id=tile_id, num_cached=loaded_count_for_tile, total_cache_size=len(self.segments_data))
            ...
            with self.loading_lock:
                if tile_processed_successfully:
                    ...
                    log_event_reader("READER_WORKER", "INFO", "TILE_PROCESSED_SUCCESS", tile_id=tile_id, num_loaded_tiles=len(self.loaded_tiles))
                else:
                    log_event_reader("READER_WORKER", "WARN", "TILE_PROCESSED_FAIL", tile_id=tile_id, path=tile_path_proto)
                if tile_id in self.queued_or_loading:
                    self.queued_or_loading.remove(tile_id)
                    log_event_reader("READER_WORKER", "DEBUG", "TILE_REMOVED_FROM_QUEUED_OR_LOADING", tile_id=tile_id, reason="processed")

            self.load_queue.task_done()
            log_event_reader("READER_WORKER", "DEBUG", "TILE_LOAD_TASK_DONE", tile_id=tile_id, queue_size=self.load_queue.qsize())

    def request_tiles(self, tile_ids: set[str]):
        """Requests tiles to be loaded by the background worker."""
        log_event_reader("READER", "INFO", "REQUEST_TILES_START", num_requested_tiles=len(tile_ids), requested_ids=list(tile_ids))
        queued_count = 0
        ...
        with self.loading_lock:
            for tile_id in tile_ids:
                if tile_id not in self.loaded_tiles and tile_id not in self.queued_or_loading:
                    ...
                    queued_count += 1
                    log_event_reader("READER", "DEBUG", "TILE_QUEUED_FOR_LOADING", tile_id=tile_id, queue_size=self.load_queue.qsize())
                elif tile_id in self.loaded_tiles:
                    already_loaded_count += 1
                    log_event_reader("READER", "TRACE", "TILE_REQUEST_SKIP_ALREADY_LOADED", tile_id=tile_id)
                else: # Already in self.queued_or_loading
                    already_queued_count +=1
                    log_event_reader("READER", "TRACE", "TILE_REQUEST_SKIP_ALREADY_QUEUED", tile_id=tile_id)

        log_event_reader("READER", "INFO", "REQUEST_TILES_END", newly_queued=queued_count, already_loaded=already_loaded_count, already_queued=already_queued_count, total_requested=len(tile_ids))
    ...
    def _add_to_cache_locked(self, osm_id, segment_data, tile_id):
        ...
        if osm_id in self.segments_data:
            ...
            log_event_reader("READER_CACHE", "TRACE", "SEGMENT_CACHE_MOVE_TO_END", segment_id=osm_id, tile_id=tile_id)
        ...
        log_event_reader("READER_CACHE", "DEBUG", "SEGMENT_ADDED_TO_CACHE", segment_id=osm_id, tile_id=tile_id, cache_size=len(self.segments_data))
    ...
    def _insert_into_rtree_locked(self, osm_id, segment_data):
        ...
        try:
            ...
            if segment_bounds:
                ...
                log_event_reader("READER_RTREE", "TRACE", "SEGMENT_INSERT_RTREE_SUCCESS", segment_id=osm_id, bounds=segment_bounds)
            else:
                log_event_reader("READER_RTREE", "WARN", "SEGMENT_INSERT_RTREE_FAIL_NO_BOUNDS", segment_id=osm_id)
        except Exception as e_rtree:
            # print(f"Warning: Failed to insert segment {osm_id} into R-tree: {e_rtree}")
            log_event_reader("READER_RTREE", "ERROR", "SEGMENT_INSERT_RTREE_EXCEPTION", segment_id=osm_id, error=str(e_rtree))
    ...
    def _remove_from_rtree_locked(self, segment_id, segment_data):
        ...
        try:
            ...
            if segment_bounds:
                ...
                log_event_reader("READER_RTREE", "TRACE", "SEGMENT_DELETE_RTREE_SUCCESS", segment_id=segment_id, bounds=segment_bounds)
            else:
                log_event_reader("READER_RTREE", "WARN", "SEGMENT_DELETE_RTREE_FAIL_NO_BOUNDS", segment_id=segment_id)
        except Exception as e:
            ...
            log_event_reader("READER_RTREE", "WARN", "SEGMENT_DELETE_RTREE_EXCEPTION", segment_id=segment_id, error=str(e))
            pass

    def _unload_tile_locked(self, tile_id):
        # Lock is held by caller (_update_loaded_tiles)
        if tile_id not in self.loaded_tiles:
            log_event_reader("READER", "DEBUG", "UNLOAD_TILE_SKIP_NOT_LOADED", tile_id=tile_id)
            return

        log_event_reader("READER", "INFO", "UNLOAD_TILE_START", tile_id=tile_id, current_cache_size=len(self.segments_data))
        ...
        if tile_id in self.queued_or_loading: # Should typically have been removed by worker already, but good to double check
             self.queued_or_loading.remove(tile_id)
             log_event_reader("READER", "DEBUG", "TILE_REMOVED_FROM_QUEUED_OR_LOADING_AT_UNLOAD", tile_id=tile_id)
        # if unloaded_count > 0: print(f"MainThread: Unloaded {unloaded_count} segments for tile {tile_id}.")
        log_event_reader("READER", "INFO", "UNLOAD_TILE_COMPLETE", tile_id=tile_id, segments_unloaded=unloaded_count, new_cache_size=len(self.segments_data), num_loaded_tiles=len(self.loaded_tiles))
    # ---------------------------
    ...
    def _update_loaded_tiles(self, lat, lon, bearing_rad=None):
        ...
        log_event_reader("READER", "DEBUG", "UPDATE_LOADED_TILES_START", lat=lat, lon=lon, bearing_rad=bearing_rad if bearing_rad is not None else "None")
        ...
        for region, bounds in REGION_BOUNDS.items():
             ...
             if min_lon_b <= lon <= max_lon_b and min_lat_b <= lat <= max_lat_b:
                 ...
                 break
        log_event_reader("READER", "DEBUG", "UPDATE_LOADED_TILES_POINT_REGION", point_region=current_point_region if current_point_region else "None", current_reader_region=self.current_region if self.current_region else "None")

        # Handle region change (needs lock for writes)
        if current_point_region != self.current_region:
            log_event_reader("READER", "INFO", "REGION_CHANGE_DETECTED", old_region=self.current_region if self.current_region else "None", new_region=current_point_region if current_point_region else "None")
            ...
            with self.loading_lock:
                ...
                log_event_reader("READER", "INFO", "REGION_CHANGE_QUEUE_CLEARED", num_cleared=num_cleared_from_queue, num_queued_or_loading_pre_clear=len(self.queued_or_loading))
            ...
            log_event_reader("READER", "INFO", "REGION_CHANGE_UNLOAD_START", num_tiles_to_unload=len(all_tiles_before_change))
            ...
            log_event_reader("READER", "INFO", "REGION_CHANGE_UNLOAD_COMPLETE")

        if not self.current_region:
            log_event_reader("READER", "WARN", "UPDATE_LOADED_TILES_NO_CURRENT_REGION", lat=lat, lon=lon)
            ...
            with self.loading_lock:
                 ...
                 if loaded_tiles_list:
                     ...
                     log_event_reader("READER", "WARN", "OUTSIDE_KNOWN_REGIONS_UNLOAD_START", num_tiles_to_unload=len(loaded_tiles_list))
            if loaded_tiles_list:
                 ...
                 with self.loading_lock:
                    ...
                    log_event_reader("READER", "WARN", "OUTSIDE_KNOWN_REGIONS_QUEUE_CLEARED", num_cleared=num_cleared_from_queue_no_region)
                 log_event_reader("READER", "WARN", "OUTSIDE_KNOWN_REGIONS_UNLOAD_COMPLETE")
