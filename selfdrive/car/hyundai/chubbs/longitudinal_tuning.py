...
class HKGLongitudinalTuning:
  ...
  # === REMOVED CODE ===
  # Removed old "calculate_emergency_factor(self, CS)" or similar discrete TTC check
  # We now read the factor from the plan.
  ...
  def calculate_limited_accel(self, accel, actuators, CS):
    ...
    # === NEW: fetch emergency factor from the planner ===
    emergency_factor = 0.0
    longitudinal_plan = None
    if hasattr(CS.sm, "all_checks"):
      # Attempt to read from the StateMachine for 'longitudinalPlan'
      plan = CS.sm['longitudinalPlan'] if 'longitudinalPlan' in CS.sm else None
      if plan and hasattr(plan, "emergencyFactor"):
        emergency_factor = plan.emergencyFactor

    # Normal jerk calculation
    ...
    # If we are accelerating, do not apply negative scaling
    # If we are decelerating, we scale it by emergency_factor
    if accel < 0.0 and emergency_factor > 0.0:
      # e.g. scale negative accel by 1 + 2*factor => up to triple at factor=1
      ef_scale = 1.0 + 2.0 * clip(emergency_factor, 0.0, 1.0)
      accel *= ef_scale
      accel_delta = accel - self.accel_last

    # Existing logic that blends negative/positive acceleration deltas with jerk-limits
    brake_blend = 0.5 - 0.5 * accel / (abs(accel) + 0.2)

    if accel < 0 and self.accel_last >= 0:
      # If we switch from non-negative to negative accel, jump-start
      self.accel_last = accel * 0.3  # or partial jump
      accel_delta = accel - self.accel_last
      # re-init ramp
      self.brake_ramp = 0.3
    ...
    # further scale if there's a big emergency factor
    # This is in addition to the earlier negative scaling
    big_emergency_scale = 1.0 + 1.2 * clip(emergency_factor, 0.0, 1.0)
    braking_ramp_rate *= big_emergency_scale

    if self.accel_last >= 0:
      transition_factor = 1.0 + 0.3 * clip(emergency_factor, 0.0, 1.0)
    ...
    emerg_resp = 0.5 + 0.5 * clip(emergency_factor, 0.0, 1.0)
    brake_smooth_factor = base_brake_smooth - 0.45 * (abs(accel) / (abs(accel) + 0.4))
    brake_smooth_factor *= (1.0 - 0.5 * emerg_resp)
    ...
    # Jerk limits
    # scale negative jerk limit by emergency_factor
    emergency_jerk_scale = 1.0 + 2.0 * clip(emergency_factor, 0.0, 1.0)
