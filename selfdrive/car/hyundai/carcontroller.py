...
class CarController(CarControllerBase, HKGLongitudinalController):
  ...
  def update(self, CC, CS, now_nanos, frogpilot_toggles):
    ...
    accel = self.calculate_accel(actuators.accel, actuators, CS, frogpilot_toggles)
    stopping = actuators.longControlState == LongCtrlState.stopping
    set_speed_in_units = hud_control.setSpeed * (CV.MS_TO_KPH if CS.is_metric else CV.MS_TO_MPH)
    self.jerk = self.calculate_and_get_jerk(actuators, self.params, CS, actuators.longControlState)
    # HUD messages
