...
from system.hardware.power_monitoring import PowerMonitoring, CAR_BATTERY_CAPACITY_uWh, \
...
def pm_patch(mocker, name, value, constant=False):
  if constant:
    mocker.patch(f"system.hardware.power_monitoring.{name}", value)
  else:
    mocker.patch(f"system.hardware.power_monitoring.{name}", return_value=value)
