...
def ensure_azure_directory_exists(conn_str, share_name, azure_path):
    from azure.storage.fileshare import ShareDirectoryClient
    dir_client = ShareDirectoryClient.from_connection_string(conn_str, share_name, azure_path)
    try:
        dir_client.create_directory()
    except Exception as e:
        if "ResourceAlreadyExists" not in str(e):
            debug_print(f"Error creating directory {azure_path}: {str(e)}")
...
def _do_upload_azure(upload_item: UploadItem, callback=None):
  ...
  within AZURE_BASE_DIR, then uploads the local file.
  Skips the upload if the file already exists.
  """
  ...
  azure_path = f"{AZURE_BASE_DIR}/{subdir_name}"
  ...
  try:
    ...
    ensure_azure_directory_exists(conn_str, AZURE_SHARE_NAME, azure_path)

    # Check if file already exists on Azure
    if azure_file_exists(conn_str, AZURE_SHARE_NAME, azure_path, filename):
      debug_print(f"File already exists on Azure, skipping upload: {filename}")
      return

    debug_print("Creating Azure directory client for upload...")
    dir_client = ShareDirectoryClient.from_connection_string(
      conn_str, AZURE_SHARE_NAME, azure_path
    )
    file_client = dir_client.get_file_client(filename)
...
def upload_handler(end_event: threading.Event) -> None:
  ...
  while not end_event.is_set():
    ...
    try:
      ...
      # If it's too old, skip and mark as done
      ...
      if age.total_seconds() > MAX_AGE:
        ...
        upload_queue.task_done()  # Remove expired item from the queue
        continue
...
def realdata_handler(end_event: threading.Event) -> None:
  """
  Scans /data/media/0/realdata/ and finds subdirectories modified within the last 24 hours.
  For each such directory, if any of the files 'rlog', 'qlog', or 'qcamera.ts' exist,
  an upload item is queued (unless it's already queued).
  """
  ...
  while not end_event.is_set():
    try:
      ...
      for subdir in os.listdir(base_path):
        ...
        # Only process directories modified within 24 hours
        mod_time = datetime.fromtimestamp(os.path.getmtime(subdir_path))
        if (now - mod_time) > age_limit:
          continue

        # Files to consider uploading
        files_to_upload = ['rlog', 'qlog', 'qcamera.ts']
        existing_files = [f for f in files_to_upload if os.path.isfile(os.path.join(subdir_path, f))]
        ...
        for filename in existing_files:
          ...
          if any(qi and qi.id == upload_id for qi in queued_items):
            continue
